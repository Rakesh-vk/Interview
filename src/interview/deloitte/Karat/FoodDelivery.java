...
class OrderManager {
    // NEW
    Map<Integer, List<Delivery>> deliveriesByOrder = new HashMap<>();
    ...
    public void  addDelivery(int orderId, Delivery delivery){
        ...
        for (Order order: orders){
            ...
            if(order.orderId==orderId) {
                ...
                for(Map.Entry<Integer, List<Delivery>> result: deliveriesByOrder.entrySet()){

                    List<Delivery> deliveries = result.getValue();
                    if(deliveries.isEmpty()){

                        deliveriesByOrder.put(orderId,new ArrayList<>());
                        deliveriesByOrder.get(orderId).add(delivery);
                    }
                    else{
                        deliveriesByOrder.get(orderId).add(delivery);
    ...
    Map<Integer,Double>  getAverageDeliveryTimeByRestaurant(){
        ...
        for(Order order:orders) {
            ...
            Integer ResturantId = order.restaurantId;
            Integer  orderId = order.orderId;
            for(Map.Entry<Integer, List<Delivery>> result: deliveriesByOrder.entrySet()){

                if(result.getKey().equals(orderId)){
                    List<Delivery> deliveriesByOrder =result.getValue();
                    Double sum =0.0;
                    for(Delivery delivery : deliveriesByOrder){

                        sum +=  delivery.getDurationMinutes();

                    }

                    if(sum>0.0) {
                        Double avg = sum / deliveriesByOrder.size();


                        avgMap.put(ResturantId, avg);
                    }
                }
