...
We are currently updating our system to include information about workouts for our members.
 As part of this update, we have introduced the Workout class, which represents a single workout
  session for a member. Each object of the Workout class has a unique ID, as well as a start time
   and end time that are represented in the number of minutes spent from the start of the day.
    You can assume that all the Workouts are from the same day.

To implement these changes, we need to add two functions to the Membership class:

2.1) The `addWorkout` function should be used to add a workout session for a given member ID.
 If the workout is stored successfully, the function should return `true`.
  If the given member does not exist while calling this function, the workout should be
   ignored and the function should return `false`.

2.2) The `getAverageWorkoutDurations` function should generate a map of member IDs onto their
 average workout durations in minutes. The returned map should include all members.
 If a member has no workouts, their value should be `null`.

To assist you in testing these new functions, we have provided the testAddWorkout
and testGetAverageWorkoutDurations functions.
*/

/*
We are building a program to manage a gym's membership.
The gym has multiple members, each with a unique ID, name, and membership status.
 The program allows gym staff to add new members, update members status, and get
...
class Membership {
    ...
    */
    ...
    public HashMap<Integer,ArrayList<Workout>> workoutTracker= new HashMap<>();

    public boolean addWorkout(Integer mamberID, Workout workout){
        boolean flag=false;
        for(Member id:members){
            if(id.memberId==mamberID){
                flag=true;
            }
        }
        ...
        if(!flag){
            return false;}
        if(workoutTracker.containsKey(mamberID)) {
            workoutTracker.get(mamberID).add(workout);
            return true;
        }else {
        ArrayList<Workout> temp = new ArrayList<>();
        workoutTracker.put(mamberID, temp);
        workoutTracker.get(mamberID).add(workout);
            return true;
        }
    }
    ...
    public Map<Integer, Double> getAverageWorkoutDurations() {
        ...
        for (Member member : members) {
            ArrayList<Workout> workouts = workoutTracker.get(member.memberId);
